...
static class Program
{
    ...
    public static void TestSpells()
    {
        ...
        Console.WriteLine("Method Enchant has the correct return and parameter types: "
                          + CheckMethodSignature(witchType, "Enchant", typeof(void),
                              new Type[] { typeof(List<ITransform>) }));
        Console.WriteLine("Method Disenchant has the correct return and parameter types: "
                          + CheckMethodSignature(witchType, "Disenchant", typeof(void),
                              new Type[] { typeof(List<ITransform>) }));
    }
        ...
        bool canRead, bool canWrite)
    {
        ...
        {
            Console.WriteLine(
                $" - Incorrect property type for {propertyName}. Expected: {expectedPropertyType}, Actual: {property.PropertyType}");
            return false;
        }
    ...
    private static bool CheckMethodSignature(Type interfaceType, string methodName, Type expectedReturnType,
        params Type[] expectedParameterTypes)
    {
        ...
        {
            Console.WriteLine(
                $" - Incorrect return type for method {methodName}. Expected: {expectedReturnType}, Actual: {method.ReturnType}");
            return false;
        }
        ...
        {
            Console.WriteLine(
                $" - Incorrect number of parameters for method {methodName}. Expected: {expectedParameterTypes.Length}, Actual: {parameters.Length}");
            return false;
        }
        ...
        for (int i = 0; i < expectedParameterTypes.Length; i++)
        {
            ...
            {
                Console.WriteLine(
                    $" - Incorrect parameter type for parameter {parameters[i].Name} in method {methodName}. Expected: {expectedParameterTypes[i]}, Actual: {parameters[i].ParameterType}");
                return false;
            }
        }
